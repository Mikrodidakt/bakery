...
The reason for using these tools is because when setting up the meta data in the layers there will be multiple git repos required. For more information on this topic please see (Meta Layers)[meta-layers.md]. How to use these tools are outside of the scope of this document but if you have not worked with repo android tool before then git submodules is recommended. There is more information available and the support is part of the git tool. The concept of using git submodules is to use a specific version of a nother project using git as repo. This means that the workspace is setup once and all external dependencies are setup once and should be fairly static during the development cycle.
